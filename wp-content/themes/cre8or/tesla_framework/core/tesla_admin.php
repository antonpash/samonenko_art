...
if(!class_exists('Tesla_admin')){
  class Tesla_admin extends TeslaFramework {
    ...
    function admin_panel_page_head() {
      ...
      wp_enqueue_script('tt-gmaps','https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&libraries=places'); //google maps api in admin head
   ...
   */
    ...
    function import_xml_ajax(){
      ...
      $multiple = !empty($_POST['step']) && $_POST['step'] > 1 ? '_' . $_POST['step'] : '';
      $total_steps = !empty($_POST['total_steps']) ? $_POST['total_steps'] : 1;
      // call the function
      $args = array(
      ...
      auto_import( $args );
      if($_POST['step'] == $total_steps && file_exists(TT_THEME_DIR . '/theme_config/demo_widgets.wie')){
        require TT_FW_DIR . '/extensions/widgetimport/widgetimport.php';
        $import_widgets_resutl = tt_process_import_file(TT_THEME_DIR . '/theme_config/demo_widgets.wie');
        tt_show_import_results($import_widgets_resutl);
      }

      if($_POST['step'] >= $total_steps)
        echo "<div class='tt_successful_import'>Demo content successfully imported. Thank you for your patience. Next you should setup your menu and widgets in Appearance menu.</div>";
      else
        echo "<div class='tt_successful_import'>File import$multiple.xml {$_POST['step']}/{$total_steps} successfully imported. Proceeding to next file.</div>";
      die();
    }
