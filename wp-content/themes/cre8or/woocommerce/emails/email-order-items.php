	...
	if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
		...
		<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
			<td class="td" style="text-align:left; vertical-align:middle; border: 1px solid #eee; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; word-wrap:break-word;"><?php
				...
				// Product name
				if ( is_object( $_product ) && ! $_product->is_visible() && $_product->get_sku() ) {
					printf( '<a href="%s">%s</a>', home_url()."/home/portfolio/".$_product->get_sku(), apply_filters( 'woocommerce_order_item_name', $item['name'], $item, false ) );
				}
				else {
