...
function woocommerce_init()
{
    ...
    class WC_Gateway_Interkassa extends WC_Payment_Gateway
    {
        ...
        {
            global $woocommerce, $wc_product_price_based_country;

            $order = new WC_Order($order_id);
            $action_adr = "https://sci.interkassa.com/";
            $result_url = str_replace('https:', 'http:', add_query_arg('wc-api', 'wc_gateway_interkassa', home_url('/')));

            if(!empty($wc_product_price_based_country))
                $exchange_rate = floatval($wc_product_price_based_country->customer->__get("exchange_rate"));

            $args = array(
                'ik_am' => round($order->order_total / ((!isset($exchange_rate) || empty($exchange_rate)) ? 1 : $exchange_rate), 2),
                'ik_cur' => 'UAH',
                'ik_co_id' => $this->merchant_id,
                'ik_pm_no' => $order_id,
                'ik_desc' => "#$order_id",
                'ik_exp' => date("Y-m-d H:i:s", time() + 24 * 3600),
				'ik_suc_u' => 'http://samonenkoart.com/checkout/order-received/?key='.$_GET['key'].'&order='.$order_id
            );
            ...
            return
                ...
                '<input type="submit" class="button alt" id="submit_interkassa_button" value="' . __('Pay for order', 'woocommerce') . '" /> ' . "\n" .
                implode("\n", $args_array) .
                '</form>';
        }
